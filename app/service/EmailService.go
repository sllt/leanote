...
func SendEMailUsingTLS(addr string, auth smtp.Auth, from string,
...
func (this *EmailService) SendEmail(to, subject, body string) (ok bool, e string) {
	...
	usetls, _ := revel.Config.Bool("email.stmp.tls.enable")

	if usetls == true {
		fmt.Println("use tls")
		err := SendEMailUsingTLS(host+":"+emailPort, auth, username, send_to, msg)
		fmt.Println(err)
		return
	}
